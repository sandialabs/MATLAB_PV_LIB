...
% accuracy of the DIRINT model improves with each additional input.
%
%% Inputs
...
%     other vector inputs. |pressure| must be >=0.
% * *|UseDelKtPrime|* - a scalar indicating if the user would like to
...
%     vectors, then time-series improvements are not used.
...
%
%% Outputs
% * *|dirintDNI|* - the modeled direct normal irradiance in W/m^2. 
%     |dirintDNI| is a column vector with the same number of
...
% model-estimated quantity is not a match.  DNI is a difficult
