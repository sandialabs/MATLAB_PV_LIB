%% pvl_reindl_2
% Estimate Direct Normal Irradiance from Global Horizontal Irradiance using the REINDL_2 model
%
%% Syntax
% |DNI = pvl_reindl_2(GHI,Z, doy)|
%
%% Description
% The Reindl-2 model estimates the diffuse fraction DF from global horizontal
...
% |pvl_reindl_1| uses the diffuse fraction to compute DHI as
%
% $$DHI = DF \times GHI $$
%
...
% with DHI estimated from GHI from the TMY3 file, using the Reindl-1 model.
...
DNI_model = pvl_reindl_2(GHI,90-AppSunEla, dayofyear);

[DNI_model, DHI_model, ~] = pvl_reindl_1(GHI,90-AppSunEla, dayofyear);
...
xlim([0 25])
legend('DHI (Reindl-1 Model)','DHI (TMY3)','Location','NE')
xlabel('Hour of Day')
ylabel('Irradiance (W/m^2)')
title({'Diffuse Horizontal Irradiance Comparison';'Albuquerque, Aug 6'},'FontSize',14)
%% References
%%
% [1] Reindl D.T., Beckman W.A., Duffie J.A., 1990. Diffuse fraction correlations.
% Solar Energy v45, pp. 1-7.
%   
%
%% See also
